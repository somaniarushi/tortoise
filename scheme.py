...
def eval_all(expressions, env):
    ...
    if expressions.rest is nil: # Tail
...
class Frame(object):
    ...
    def make_child_frame(self, formals, vals):
        ...
        child= Frame(self)
        while formals:
            name, value = formals.first, vals.first
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 9
        fn = do_lambda_form(Pair(target.rest, expressions.rest), env)
...
def do_lambda_form(expressions, env):
    ...
    # Create LambdaProcedure Instance
...
def make_let_frame(bindings, env):
    ...
    # BEGIN PROBLEM 14
    # binding would be something like Pair (Pair, Pair)
    ...
    while bindings is not nil:
        ...
        name, value = current_binding.first, scheme_eval(current_binding.rest.first, env)
        formals, values = Pair(name, formals), Pair(value, values)
